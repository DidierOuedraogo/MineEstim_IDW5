...
if uploaded_file is True or uploaded_file:  # Check for both boolean and file object
    ...
    try:
        ...
        with st.sidebar.expander("Statistiques avant filtrage", expanded=False):
            st.write(f"Nombre total de lignes: {len(filtered_df)}")
            st.write(f"Valeurs manquantes dans {col_x}: {filtered_df[col_x].isna().sum()}")
            st.write(f"Valeurs manquantes dans {col_y}: {filtered_df[col_y].isna().sum()}")
            st.write(f"Valeurs manquantes dans {col_z}: {filtered_df[col_z].isna().sum()}")
            st.write(f"Valeurs manquantes dans {col_value}: {filtered_df[col_value].isna().sum()}")
            
            valid_mask = (filtered_df[col_x].notna() & 
                          filtered_df[col_y].notna() & 
                          filtered_df[col_z].notna() & 
                          filtered_df[col_value].notna())
            st.write(f"Nombre de lignes avec toutes les valeurs présentes: {valid_mask.sum()}")
        ...
        if col_domain != '-- Aucun --' and domain_filter_value is not None:
            ...
            with st.sidebar.expander("Statistiques après filtrage de domaine", expanded=False):
                st.write(f"Nombre de lignes après filtrage: {len(filtered_df)}")
                st.write(f"Filtre appliqué: {col_domain} {domain_filter_type} {domain_filter_value}")
        ...
        # Convertir les valeurs en nombres (gestion robuste des erreurs)
        ...
        # Diagnostics de conversion
        conversion_issues = []
        
        # Créer la liste des composites
        for idx, row in filtered_df.iterrows():
            x_val = safe_float_convert(row[col_x])
            y_val = safe_float_convert(row[col_y])
            z_val = safe_float_convert(row[col_z])
            value_val = safe_float_convert(row[col_value])
            
            # Vérifier si toutes les valeurs nécessaires sont disponibles
            if x_val is not None and y_val is not None and z_val is not None and value_val is not None:
                composite = {
                    'X': x_val,
                    'Y': y_val,
                    'Z': z_val,
                ...
                if density_method == "Variable (colonne)" and density_column and density_column in row:
                    density_val = safe_float_convert(row[density_column])
                ...
                issues = []
                if x_val is None: issues.append(f"{col_x}")
                if y_val is None: issues.append(f"{col_y}")
                if z_val is None: issues.append(f"{col_z}")
                if value_val is None: issues.append(f"{col_value}")
                conversion_issues.append(f"Ligne {idx+1}: Problème avec {', '.join(issues)}")
        
        # Afficher les diagnostics de conversion
        with st.sidebar.expander("Statistiques de conversion", expanded=False):
            st.write(f"Nombre de composites créés: {len(composites_data)}")
            if conversion_issues:
                st.write(f"Problèmes de conversion détectés sur {len(conversion_issues)} lignes")
                if len(conversion_issues) <= 10:
                    for issue in conversion_issues:
                        st.write(issue)
                else:
                    st.write("Premiers problèmes de conversion:")
                    for issue in conversion_issues[:10]:
                        st.write(issue)
                    st.write(f"... et {len(conversion_issues)-10} autres.")
